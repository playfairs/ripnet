         ...
         ping_result_t *result)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        printf("  %d bytes from %s: icmp_seq=%d ttl=64 "
               "time=%.2f ms\n",
               64,
               inet_ntoa(dest_addr.sin_addr),
               i + 1,
               ...
               int end_ip)
{
    ...
    for (int i = start_ip; i <= end_ip && i < start_ip + 16;
         i++)
    {
        char host[32];
        snprintf(host, sizeof(host), "%d", i);
        printf(
            "  Probe %s: using hostname resolution only\n",
            host);
    }

    printf("  " COLOR_GREEN
           "Sweep completed with lightweight host "
           "checks\n" COLOR_RESET);
    return 0;
}
...
int ping_flood(const char *hostname, int duration_sec)
{
    ...
    printf(COLOR_BOLD COLOR_CYAN
           "PING FLOOD\n" COLOR_RESET);
    printf("  " COLOR_RED
           "Ping flood is a potentially harmful "
           "operation\n" COLOR_RESET);
    printf("  " COLOR_YELLOW
           "This command requires explicit "
           "authorization\n" COLOR_RESET);
    return -1;
}
             ...
             ping_result_t *result)
{
    ...
    int result_code = connect(sockfd,
    ...
    if (result_code == 0)
    {
        printf("  Port " COLOR_GREEN "OPEN" COLOR_RESET
               " - RTT: %.2f ms\n",
               rtt);
        result->success = 1;
    ...
    {
        printf("  Port " COLOR_RED "CLOSED" COLOR_RESET
               " or filtered\n");
        result->success = 0;
    }
...
void print_ping_results(const ping_result_t *result)
{
    ...
    printf("  Packets: Sent = %" PRIu64
           ...
           result->packet_loss);
    printf("  RTT: Min = %.2f ms, Max = %.2f ms, Avg = "
           "%.2f ms\n",
           result->min_rtt,
           result->max_rtt,
           result->avg_rtt);
