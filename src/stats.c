...
int list_interfaces(interface_stats_t **stats, int *count)
{
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
    {
        ...
        if (!found)
        {
            ...
            result = xrealloc(
                result,
                num_interfaces * sizeof(interface_stats_t));
            memset(&result[num_interfaces - 1],
                   0,
                   sizeof(interface_stats_t));
            strncpy(result[num_interfaces - 1].name,
                    ifa->ifa_name,
                    255);
            result[num_interfaces - 1].name[255] = '\0';
            result[num_interfaces - 1].is_up =
                (ifa->ifa_flags & IFF_UP) != 0;
            result[num_interfaces - 1].is_running =
            ...
            if (sockfd >= 0)
            {
                ...
                if (ioctl(sockfd, SIOCGIFHWADDR, &ifr) == 0)
                {
                    ...
                    snprintf(
                        result[num_interfaces - 1].mac_addr,
                        18,
                        ...
                        interface_stats_t *stats)
{
    ...
    snprintf(path,
             sizeof(path),
             "/sys/class/net/%s/statistics/rx_dropped",
             iface);
    fp = fopen(path, "r");
    if (fp)
    {
...
void print_interface_stats(const interface_stats_t *stats)
{
    ...
    printf("  Status: %s, %s\n",
           stats->is_up ? "UP" : "DOWN",
           stats->is_running ? "RUNNING" : "NOT RUNNING");
    printf("  MAC: %s\n", stats->mac_addr);
    printf("  MTU: %u\n", stats->mtu);
    printf("  RX: %" PRIu64 " packets (%s), %" PRIu64
           " errors, %" PRIu64 " dropped\n",
           stats->rx_packets,
           rx_buf,
           stats->rx_errors,
           stats->rx_dropped);
    printf("  TX: %" PRIu64 " packets (%s), %" PRIu64
           " errors, %" PRIu64 " dropped\n",
           stats->tx_packets,
           tx_buf,
           stats->tx_errors,
