...
int firewall_list(firewall_ruleset_t *ruleset)
{
           ...
           && ruleset->rule_count < 1024)
    {
        ...
        firewall_rule_t *rule =
            &ruleset->rules[ruleset->rule_count];
        memset(rule, 0, sizeof(firewall_rule_t));

        char pkts[32], bytes[32], target[64], prot[16],
            opt[32], in[32], out[32], source[64],
            destination[64], opts[256];
        ...
        if (sscanf(line,
            ...
            >= 5)
        {
            strncpy(rule->packets,
                    pkts,
                    sizeof(rule->packets) - 1);
            strncpy(rule->bytes,
                    bytes,
                    sizeof(rule->bytes) - 1);
            strncpy(rule->target,
                    target,
                    sizeof(rule->target) - 1);
            strncpy(rule->protocol,
                    prot,
                    sizeof(rule->protocol) - 1);
            strncpy(rule->source,
                    source,
                    sizeof(rule->source) - 1);
            strncpy(rule->destination,
                    destination,
                    sizeof(rule->destination) - 1);
            strncpy(rule->options,
                    opts,
                    sizeof(rule->options) - 1);
...
#else
    fprintf(stderr,
            "Firewall not supported on this platform\n");
    return -1;
#endif
...
#elif defined(__APPLE__)
    printf("  " COLOR_YELLOW
           "Use pfctl to add rules on macOS\n" COLOR_RESET);
    return -1;
#else
    printf("  " COLOR_YELLOW
           "Firewall add not supported on this "
           "platform\n" COLOR_RESET);
    return -1;
#endif
}
...
#else
    printf("  " COLOR_YELLOW
           "Firewall delete not supported on this "
           "platform\n" COLOR_RESET);
    return -1;
#endif
}
...
#else
    printf("  " COLOR_YELLOW
           "Firewall flush not supported on this "
           "platform\n" COLOR_RESET);
    return -1;
#endif
}
...
#else
    printf("  " COLOR_YELLOW
           "Firewall status not supported on this "
           "platform\n" COLOR_RESET);
#endif
...
int firewall_log(int enable)
{
    ...
    printf(COLOR_BOLD COLOR_CYAN
           "FIREWALL LOG\n" COLOR_RESET);
    printf("  " COLOR_YELLOW
           "Firewall logging requires specific "
           "configuration\n" COLOR_RESET);
    return -1;
}
...
int firewall_block_ip(const char *ip)
{
    ...
    return 0;
#else
    printf("  " COLOR_YELLOW "IP blocking not supported on "
                             "this platform\n" COLOR_RESET);
    return -1;
#endif
}
...
int firewall_unblock_ip(const char *ip)
{
    ...
    snprintf(command,
             sizeof(command),
             "sudo iptables -D INPUT -s %s -j DROP",
             ip);
    system(command);
...
#else
    printf("  " COLOR_YELLOW
           "IP unblocking not supported on this "
           "platform\n" COLOR_RESET);
    return -1;
#endif
}
...
int firewall_block_port(int port, const char *protocol)
{
    ...
    snprintf(
        command,
        sizeof(command),
        "sudo iptables -A INPUT -p %s --dport %d -j DROP",
        protocol,
        port);
    system(command);
...
#else
    printf("  " COLOR_YELLOW
           "Port blocking not supported on this "
           "platform\n" COLOR_RESET);
    return -1;
#endif
}
...
int firewall_unblock_port(int port, const char *protocol)
{
    ...
    snprintf(
        command,
        sizeof(command),
        "sudo iptables -D INPUT -p %s --dport %d -j DROP",
        protocol,
        port);
    system(command);
...
#else
    printf("  " COLOR_YELLOW
           "Port unblocking not supported on this "
           "platform\n" COLOR_RESET);
    return -1;
#endif
}
