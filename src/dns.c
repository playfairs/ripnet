...
int dns_lookup(const char *hostname,
               dns_lookup_result_t *result)
{
    ...
    printf("  Hostname: " COLOR_BOLD "%s\n" COLOR_RESET,
           result->hostname);
    printf("  IP Address: " COLOR_BOLD COLOR_GREEN
           "%s\n" COLOR_RESET,
           result->ip_address);
    printf("  Response Time: " COLOR_BOLD COLOR_YELLOW
           "%.2f ms\n" COLOR_RESET,
           (double)result->response_time_ms);

    return 0;
}

int dns_reverse_lookup(const char *ip_address,
                       char *hostname,
                       size_t hostname_len)
{
    ...
    printf("  Hostname: " COLOR_BOLD COLOR_GREEN
           "%s\n" COLOR_RESET,
           hostname);
...
int dns_server_test(const char *dns_server,
                    dns_server_test_result_t *result)
{
    dns_lookup_result_t lookup;
    ...
    {
        strncpy(result->dns_server,
                dns_server,
                sizeof(result->dns_server) - 1);
        result->queries_sent = 1;
        result->queries_failed = 0;
        result->avg_response_time =
            (double)lookup.response_time_ms;
        result->success = 1;
        printf("  " COLOR_GREEN
               "Server responded in %.2f ms\n" COLOR_RESET,
               result->avg_response_time);
        return 0;
    }
...
int dns_trace(const char *hostname)
{
    ...
    {
        printf("  " COLOR_GREEN
               "Resolved through standard resolver to "
               "%s\n" COLOR_RESET,
               lookup.ip_address);
        return 0;
    }
...
int dns_bruteforce(const char *domain,
...
int dns_zone_transfer(const char *domain,
                      const char *dns_server)
{
    ...
    printf("  DNS Server: %s\n", dns_server);
    printf("  " COLOR_YELLOW
           "Zone transfer is not enabled by default and "
           "was not attempted\n" COLOR_RESET);
    return -1;
}
...
int dnssec_verify(const char *domain)
{
    ...
    if (dns_lookup(domain, &lookup) == 0)
    {
        printf(
            "  " COLOR_GREEN
            "DNS resolution succeeded for %s\n" COLOR_RESET,
            domain);
        return 0;
    }
...
int dns_cache_flush(void)
{
    ...
    system("sudo dscacheutil -flushcache && sudo killall "
           "-HUP mDNSResponder");
    printf(COLOR_BOLD COLOR_CYAN
           "DNS CACHE FLUSH\n" COLOR_RESET);
    printf("  " COLOR_GREEN
    ...
    printf(COLOR_BOLD COLOR_CYAN
           "DNS CACHE FLUSH\n" COLOR_RESET);
    printf("  " COLOR_GREEN
           "DNS cache flushed on Linux\n" COLOR_RESET);
    return 0;
#else
    printf(COLOR_BOLD COLOR_CYAN
           "DNS CACHE FLUSH\n" COLOR_RESET);
    printf("  " COLOR_YELLOW
           "DNS cache flush not supported on this "
           "platform\n" COLOR_RESET);
    return -1;
#endif
}
