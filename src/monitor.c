...
{
    strncpy(monitor_interface,
            interface,
            sizeof(monitor_interface) - 1);
    monitor_running = 1;
...
int monitor_status(monitor_snapshot_t *snapshot)
{
    ...
    {
        fprintf(stderr,
                "Could not get interface statistics\n");
        return -1;
    }
    ...
    {
        uint64_t rx_delta = snapshot->current.rx_bytes
                            - snapshot->previous.rx_bytes;
        uint64_t tx_delta = snapshot->current.tx_bytes
                            - snapshot->previous.tx_bytes;

        snapshot->current.rx_rate_mbps =
            (double)rx_delta / 1000000.0;
        snapshot->current.tx_rate_mbps =
            (double)tx_delta / 1000000.0;

        uint64_t packet_delta =
            (snapshot->current.rx_packets
             + snapshot->current.tx_packets)
            - (snapshot->previous.rx_packets
               + snapshot->previous.tx_packets);
        uint64_t error_delta =
            (snapshot->current.rx_errors
             + snapshot->current.tx_errors)
            - (snapshot->previous.rx_errors
               + snapshot->previous.tx_errors);

        if (packet_delta > 0)
        {
            snapshot->current.packet_loss =
                (double)error_delta / packet_delta * 100.0;
        }
        else
        {
...
int monitor_log(const char *interface, const char *log_path)
{
    ...
    printf("  Log path: %s\n", log_path);
    printf("  " COLOR_YELLOW
           "Monitor logging requires file I/O "
           "implementation\n" COLOR_RESET);
    return -1;
}
                   ...
                   const char *export_path)
{
    ...
    printf("  " COLOR_YELLOW "Monitor export requires data "
                             "serialization\n" COLOR_RESET);
    return -1;
}
    ...
    const monitor_snapshot_t *snapshot)
{
    ...
    format_bytes(snapshot->current.rx_bytes,
                 rx_buf,
                 sizeof(rx_buf));
    format_bytes(snapshot->current.tx_bytes,
                 tx_buf,
                 sizeof(tx_buf));
