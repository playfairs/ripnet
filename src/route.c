...
int route_table(route_table_t *table)
{
           ...
           && table->entry_count < 256)
    {
        ...
        if (sscanf(line,
            ...
            == 4)
        {
            strncpy(entry->destination,
                    dest,
                    sizeof(entry->destination) - 1);
            strncpy(entry->gateway,
                    gateway,
                    sizeof(entry->gateway) - 1);
            strncpy(entry->interface,
                    iface,
                    sizeof(entry->interface) - 1);
            entry->flags = atoi(flags);
    ...
    fprintf(
        stderr,
        "Routing table not supported on this platform\n");
    return -1;
#endif
...
int route_add(const char *destination,
              const char *gateway,
              const char *netmask,
              const char *interface)
{
    ...
    return 0;
#else
    printf("  " COLOR_YELLOW "Route add not supported on "
                             "this platform\n" COLOR_RESET);
    return -1;
#endif
}
...
#else
    printf("  " COLOR_YELLOW
           "Route delete not supported on this "
           "platform\n" COLOR_RESET);
    return -1;
#endif
}
...
int route_get(const char *destination, route_entry_t *route)
{
    ...
    {
        if (strcmp(table.entries[i].destination,
                   destination)
                == 0
            || strcmp(table.entries[i].destination,
                      "0.0.0.0")
                   == 0)
        {
            memcpy(route,
    ...
    printf("  " COLOR_YELLOW
           "No route entry matched the requested "
           "destination\n" COLOR_RESET);
    return -1;
}
...
int route_trace(const char *destination)
{
    ...
    printf("  Destination: %s\n", destination);
    printf("  " COLOR_GREEN
           "Route trace completed using the current "
           "routing table\n" COLOR_RESET);
    return 0;
}
...
int route_monitor(void)
{
    printf(COLOR_BOLD COLOR_CYAN
           "ROUTE MONITOR\n" COLOR_RESET);
    printf("  " COLOR_GREEN
           "Route monitoring uses the current routing "
           "table snapshot\n" COLOR_RESET);
    return 0;
}
