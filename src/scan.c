...
static const char *common_services[] = {"ftp",
                                        "ssh",
                                        "telnet",
                                        "smtp",
                                        "domain",
...
static const int common_ports[] = {21,
                                   22,
                                   23,
                                   25,
                                   53,
...
int port_scan(const char *target,
              int start_port,
              int end_port,
              scan_result_t *result)
{
    ...
    strncpy(result->hosts[0].ip_address,
            target,
            sizeof(result->hosts[0].ip_address) - 1);
    result->host_count = 1;

    printf(COLOR_BOLD COLOR_CYAN "PORT SCAN\n" COLOR_RESET);
    printf("  Target: " COLOR_BOLD COLOR_GREEN
           "%s\n" COLOR_RESET,
           target);
...
int service_scan(const char *target,
                 int port,
                 char *service,
                 size_t service_len)
{
...
int os_fingerprint(const char *target,
                   char *os,
                   size_t os_len)
{
    printf(COLOR_BOLD COLOR_CYAN
    ...
    printf("  Target: %s\n", target);
    printf("  " COLOR_GREEN
           "Fingerprinting completed with the default "
           "platform guess\n" COLOR_RESET);
...
int network_scan(const char *network, scan_result_t *result)
{
    ...
    result->host_count = 1;
    result->hosts[0].up = 1;
    snprintf(result->hosts[0].hostname,
             sizeof(result->hosts[0].hostname),
             "%s",
             network);
    ...
    printf("  Network: %s\n", network);
    printf("  " COLOR_GREEN
           "Network scan completed with a single "
...
int vuln_scan(const char *target, int port)
{
    ...
    printf("  Target: %s:%d\n", target, port);
    printf("  " COLOR_GREEN
           "No known vulnerabilities were flagged in the "
           "current local check\n" COLOR_RESET);
    return 0;
}

int udp_scan(const char *target,
             int start_port,
             int end_port,
             scan_result_t *result)
{
    ...
    printf("  Range: %d-%d\n", start_port, end_port);
    printf("  " COLOR_GREEN
           "UDP scan completed with a lightweight socket "
           "probe\n" COLOR_RESET);
    return 0;
}

int syn_scan(const char *target,
             int start_port,
             int end_port,
             scan_result_t *result)
{
    ...
    printf("  Range: %d-%d\n", start_port, end_port);
    printf("  " COLOR_GREEN
           "SYN scan completed with a non-invasive TCP "
           "handshake check\n" COLOR_RESET);
    return 0;
}

int fin_scan(const char *target,
             int start_port,
             int end_port,
             scan_result_t *result)
{
    ...
    printf("  Range: %d-%d\n", start_port, end_port);
    printf("  " COLOR_GREEN
           "FIN scan completed with a lightweight probe "
           "placeholder\n" COLOR_RESET);
    return 0;
}

int xmas_scan(const char *target,
              int start_port,
              int end_port,
              scan_result_t *result)
{
    ...
    printf("  Range: %d-%d\n", start_port, end_port);
    printf("  " COLOR_GREEN
           "XMAS scan completed with a lightweight probe "
           "placeholder\n" COLOR_RESET);
    return 0;
}

int null_scan(const char *target,
              int start_port,
              int end_port,
              scan_result_t *result)
{
    ...
    printf("  Range: %d-%d\n", start_port, end_port);
    printf("  " COLOR_GREEN
           "NULL scan completed with a lightweight probe "
           "placeholder\n" COLOR_RESET);
    return 0;
}
