                   ...
                   bandwidth_result_t *result)
{
    ...
    bandwidth_stats_t *bw =
        &result->interfaces[result->interface_count];
    strncpy(bw->interface,
            interface,
            sizeof(bw->interface) - 1);

    uint64_t rx_delta =
        stats_after.rx_bytes - stats_before.rx_bytes;
    uint64_t tx_delta =
    ...
    bw->total_rate_mbps =
        bw->rx_rate_mbps + bw->tx_rate_mbps;

    result->interface_count++;
    result->total_bandwidth_mbps += bw->total_rate_mbps;

    printf("  RX Rate: " COLOR_BOLD COLOR_GREEN
           "%.2f Mbps\n" COLOR_RESET,
           bw->rx_rate_mbps);
    printf("  TX Rate: " COLOR_BOLD COLOR_GREEN
           "%.2f Mbps\n" COLOR_RESET,
           bw->tx_rate_mbps);
    printf("  Total: " COLOR_BOLD COLOR_CYAN
           "%.2f Mbps\n" COLOR_RESET,
           bw->total_rate_mbps);
                        ...
                        bandwidth_result_t *result)
{
    ...
    printf("  Server: %s\n", server);
    printf("  " COLOR_YELLOW
           "Speedtest requires external service "
           "integration\n" COLOR_RESET);
    return -1;
}
                      ...
                      int interval_sec)
{
    ...
    while (1)
    {
        ...
        if (get_interface_stats(interface, &stats_after)
            < 0)
        {
            fprintf(stderr,
                    "Could not get interface statistics\n");
        ...
        double rx_rate_mbps =
            (double)(rx_delta / interval_sec) / 1000000.0;
        double tx_rate_mbps =
            (double)(tx_delta / interval_sec) / 1000000.0;
...
int bandwidth_history(const char *interface, int hours)
{
    ...
    printf("  Hours: %d\n", hours);
    printf("  " COLOR_YELLOW
           "Bandwidth history requires data logging "
           "system\n" COLOR_RESET);
    return -1;
}
...
int bandwidth_limit(const char *interface, uint64_t max_bps)
{
    ...
    printf("  " COLOR_YELLOW
           "Bandwidth limiting requires tc (traffic "
           "control)\n" COLOR_RESET);
    return -1;
}
                     ...
                     uint64_t upload_bps)
{
    ...
    printf(COLOR_BOLD COLOR_CYAN
           "BANDWIDTH SHAPER\n" COLOR_RESET);
    printf("  Interface: %s\n", interface);
    printf("  Download: %" PRIu64 " bps, Upload: %" PRIu64
           " bps\n",
           download_bps,
           upload_bps);
    printf("  " COLOR_YELLOW
           "Bandwidth shaping requires tc (traffic "
           "control)\n" COLOR_RESET);
    return -1;
}
