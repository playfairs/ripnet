...
int netstat_connections(netstat_result_t *result)
{
...
#else
    fprintf(stderr,
            "Netstat not supported on this platform\n");
    return -1;
#endif
           ...
           && result->connection_count < 4096)
    {
        ...
        if (token == NULL)
            continue;
        strncpy(conn->protocol,
                token,
                sizeof(conn->protocol) - 1);

        token = strtok(NULL, " \t\n");
        if (token == NULL)
            continue;
        strncpy(conn->local_address,
                token,
                sizeof(conn->local_address) - 1);

        token = strtok(NULL, " \t\n");
        if (token == NULL)
            continue;
        strncpy(conn->remote_address,
                token,
                sizeof(conn->remote_address) - 1);
        ...
        if (strcmp(conn->protocol, "tcp") == 0)
            result->tcp_count++;
        else if (strcmp(conn->protocol, "udp") == 0)
            result->udp_count++;

        if (strstr(conn->state, "LISTEN") != NULL)
            result->listening_count++;
        if (strstr(conn->state, "ESTABLISHED") != NULL)
...
int netstat_listening(netstat_result_t *result)
{
    ...
    {
        if (strstr(result->connections[i].state, "LISTEN")
            != NULL)
        {
            printf("  %s %s %s\n",
...
int netstat_process(int pid, netstat_result_t *result)
{
    ...
    printf(COLOR_BOLD COLOR_CYAN
           "NETSTAT PROCESS\n" COLOR_RESET);
    printf("  " COLOR_YELLOW
           "Process-specific netstat requires lsof or ss "
...
#else
    printf(
        "  Route table not supported on this platform\n");
#endif
...
int netstat_interface(const char *iface)
{
    ...
    system(command);
#else
    printf("  Interface statistics not supported on this "
           "platform\n");
#endif
...
int netstat_group(const char *group)
{
    ...
    printf(COLOR_BOLD COLOR_CYAN
           "NETWORK GROUPS\n" COLOR_RESET);
    printf("  " COLOR_YELLOW
           "Network groups require multicast group "
           "support\n" COLOR_RESET);
    return -1;
}
...
int netstat_timer(void)
{
    printf(COLOR_BOLD COLOR_CYAN
           "NETWORK TIMERS\n" COLOR_RESET);
    printf("  " COLOR_YELLOW
           "Network timers require kernel parameter "
           "access\n" COLOR_RESET);
    return -1;
}
...
void print_netstat_results(const netstat_result_t *result)
{
    ...
    printf("  Connections:\n");
    for (int i = 0; i < result->connection_count && i < 50;
         i++)
    {
