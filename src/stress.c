...
static void *tcp_worker(void *arg)
{
    ...
    while (!*ctx->should_stop)
    {
        ...
        int request_len = snprintf(full_request,
                                   sizeof(full_request),
                                   "%s%s%s",
                                   request,
                                   ctx->config->host,
        ...
        if (sent > 0)
        {
            ...
            char response[8192];
            ssize_t received = recv(wctx.sockfd,
                                    response,
                                    sizeof(response),
                                    0);
            if (received > 0)
            {
                ...
                ctx->result->total_bytes_received +=
                    received;
            }
        }

        pthread_mutex_lock(ctx->latency_mutex);
        if (*ctx->latency_index
            < ctx->config->concurrency * 1000)
        {
            ctx->latencies[*ctx->latency_index] =
                wctx.latency_ms;
            (*ctx->latency_index)++;
        }
        ...
        if (wctx.latency_ms < ctx->result->min_latency_ms
            || ctx->result->min_latency_ms == 0)
        {
                   ...
                   stress_result_t *result)
{
    ...
    threads =
        xmalloc(config->concurrency * sizeof(pthread_t));
    latencies = xmalloc(config->concurrency * 1000
                        * sizeof(double));
    ...
    printf(COLOR_RESET "  Target: " COLOR_BOLD COLOR_GREEN
                       "%s:%d\n",
           config->host,
           config->port);
    printf(COLOR_RESET
           "  Duration: " COLOR_BOLD COLOR_YELLOW
           "%d seconds\n",
           config->duration_sec);
    printf(COLOR_RESET
           "  Concurrency: " COLOR_BOLD COLOR_MAGENTA
           "%d workers\n",
           config->concurrency);
    printf("\n");

    for (int elapsed = 0; elapsed < config->duration_sec;
         elapsed++)
    {
        double conn_rate = (double)result->total_connections
                           / (elapsed + 1);
        double success_rate =
            result->total_connections > 0
                ? (double)result->successful_connections
                      / result->total_connections * 100.0
                : 0.0;
        char rate_buf[32];
        format_rate(result->total_bytes_sent * 8
                        / (elapsed + 1),
                    rate_buf,
                    sizeof(rate_buf));

        printf(COLOR_BOLD
               "\r[" COLOR_CYAN "%02d/%02d" COLOR_RESET
               "] " COLOR_GREEN "Conn: %" PRIu64 COLOR_RESET
               " | " COLOR_YELLOW
               "Success: %" PRIu64 COLOR_RESET
               " (%.1f%%) | " COLOR_RED
               "Failed: %" PRIu64 COLOR_RESET
               " | " COLOR_MAGENTA
               "Rate: %.1f/s" COLOR_RESET " | " COLOR_BLUE
               "BW: %s" COLOR_RESET,
               elapsed + 1,
               config->duration_sec,
    ...
    result->throughput_mbps =
        ...
        * 8.0 / (config->duration_sec * 1000000.0);
                    ...
                    stress_result_t *result)
{
    ...
    (void)result;
    fprintf(stderr,
            "HTTP stress test not yet implemented\n");
    return -1;
}
...
void print_stress_results(const stress_result_t *result)
{
    ...
    format_bytes(result->total_bytes_sent,
                 sent_buf,
                 sizeof(sent_buf));
    format_bytes(result->total_bytes_received,
                 recv_buf,
                 sizeof(recv_buf));
    ...
    double success_rate =
        result->total_connections > 0
            ...
            : 0.0;
    double conn_rate =
        result->total_connections / result->duration_sec;
    ...
    printf(COLOR_RESET
           ...
           result->duration_sec);
    printf(COLOR_RESET
           "  Total Conn:      " COLOR_BOLD COLOR_GREEN
           "%" PRIu64 "\n",
           result->total_connections);
    printf(COLOR_RESET
           "  Successful:      " COLOR_BOLD COLOR_GREEN
           "%" PRIu64 COLOR_RESET " (%.2f%%)\n",
           result->successful_connections,
           success_rate);
    printf(COLOR_RESET
           "  Failed:          " COLOR_BOLD COLOR_RED
           "%" PRIu64 "\n",
           result->failed_connections);
    printf(COLOR_RESET
           "  Conn Rate:       " COLOR_BOLD COLOR_MAGENTA
           "%.1f conn/s\n",
           conn_rate);
    printf(COLOR_RESET
           "  Bytes Sent:      " COLOR_BOLD COLOR_BLUE
           "%s\n",
           sent_buf);
    printf(COLOR_RESET
           "  Bytes Received:  " COLOR_BOLD COLOR_BLUE
           "%s\n",
           recv_buf);
    printf(COLOR_RESET
           "  Throughput:      " COLOR_BOLD COLOR_BLUE
           "%.2f Mbps\n",
           result->throughput_mbps);
    printf(COLOR_RESET "  Latency (ms):\n");
    printf(COLOR_RESET
           "    Min:           " COLOR_BOLD COLOR_YELLOW
           "%.2f\n",
           result->min_latency_ms);
    printf(COLOR_RESET
           "    Max:           " COLOR_BOLD COLOR_YELLOW
           "%.2f\n",
           result->max_latency_ms);
    printf(COLOR_RESET
           "    Avg:           " COLOR_BOLD COLOR_YELLOW
           "%.2f\n",
           result->avg_latency_ms);
    printf(COLOR_RESET
           "    P50:           " COLOR_BOLD COLOR_YELLOW
           "%.2f\n",
           result->p50_latency_ms);
    printf(COLOR_RESET
           "    P95:           " COLOR_BOLD COLOR_YELLOW
           "%.2f\n",
           result->p95_latency_ms);
    printf(COLOR_RESET
           "    P99:           " COLOR_BOLD COLOR_YELLOW
           "%.2f\n",
           result->p99_latency_ms);
    printf(COLOR_RESET "\n");
}
                 ...
                 uint64_t packets_per_sec)
{
    ...
    fprintf(stderr,
            "Packet flood requires raw socket access and "
            "is only available on authorized systems\n");
    return -1;
}
...
int detect_vulnerabilities(const char *target_ip, int port)
{
    ...
    (void)port;
    fprintf(stderr,
            "Vulnerability scanning requires explicit "
